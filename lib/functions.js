...
const simih = async (text) => {
	try {
		const sami = await fetch(`https://api-xcoders.xyz/api/fun/simi?text=${pesan}&apikey=hnslrl1882`, {method: 'GET'})
		const res = await sami.json()
		return res.result
	} catch {
		return 'Maaf kak simi tidak mengerti apa yang kakak bicarakan'
	}
}
...
const banner = cfonts.render(('OCTAVE|TECH|ID'), {
    font: 'chrome',
    color: 'candy',
    align: 'center',
    gradient: ["red","yellow"],
